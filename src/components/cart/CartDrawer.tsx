...
interface CartDrawerProps {
  ...
  city?: any;
  defaultView?: 'cart' | 'checkout'; // Kept for prop compatibility but unused
}
...
export default function CartDrawer({ open, onOpenChange, city }: CartDrawerProps) {
  ...
  const handleCheckout = () => {
    ...
    onOpenChange(false);
    const cityName = city?.name ? encodeURIComponent(city.name) : 'Lagos';
    const cityId = city?._id || '';
    router.push(`/checkout?cityName=${cityName}&cityId=${cityId}`);
  };
